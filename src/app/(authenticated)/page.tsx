...
import {
  ...
  KpiSkeleton,
  PayPairSkeleton,
} from "@/components/analytics/dashboard-skeletons";
...
/**
 * Dashboard shell. Each data-driven section fetches its own slice and streams in
 * under its own <Suspense> boundary, so cheap aggregates (KPIs, distributions,
 * headcount) paint immediately instead of being blocked by the heaviest query
 * (the pay breakdowns, which scan ~10k current salary rows). The analytics
 * getters are wrapped in React `cache()`, so slices shared by two sections
 * (`stats`, `pay`) still run a single query per request.
 */
  ...
  return (
    <div className="space-y-4">
      ...
      <div id="dashboard-report" className="space-y-4">
        ...
        <Suspense fallback={<PayPairSkeleton />}>
          <PayPairSection />
        </Suspense>
...
async function DistributionSection() {
  const [stats, genderRows] = await Promise.all([
    getDashboardStats(),
    getGenderBreakdown(),
  ]);
...
async function PayPairSection() {
  const pay = await getPayBreakdowns();
  return (
...
async function HeadcountSection() {
  const headcountByDept = await getHeadcountByDepartment();
  return <HeadcountByDepartmentChart data={headcountByDept} />;
}

async function PayDeptSection() {
  const pay = await getPayBreakdowns();
  return (
    <PayBreakdownCard
