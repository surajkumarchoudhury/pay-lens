...
/**
 * Multi-select level facet. Mirrors the status popover (title, list, Apply) with
 * checkboxes, since filtering by a span of seniority (e.g. L4–L6) is common.
 * State of record is the URL (`level`, comma-separated); the popover holds a
 * draft until Apply.
 */
export function LevelFilter({ value }: { value: EmployeeLevelId[] }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState<EmployeeLevelId[]>(value);

  const isActive = value.length > 0;

  const toggle = (id: EmployeeLevelId) =>
    setDraft((prev) =>
  ...
  const onApply = () => {
    ...
    if (draft.length) params.set("level", draft.join(","));
  ...
  return (
    ...
    >
      ...
      <Popover.Portal>
        ...
        >
          <div className="mb-3 text-sm font-semibold">Level</div>
